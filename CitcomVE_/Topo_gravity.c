   ...
   
{
    ...
    for(els=1;els<=E->mesh.snel;els++) {
	    ...
	    get_global_1d_shape_fn(E,E->mesh.levmax,el,&GM,&dGammax,1);
	    get_global_1d_shape_fn(E,E->mesh.levmax,elb,&GMb,&dGammabx,1);
    ...
    int ii;
{
    ...
    for(snode=1;snode<=E->mesh.nsf;snode++)   {
		...
		tpg[snode]  = -Szz[node];
		tpgb[snode] = Szz[node+E->mesh.noz-1]; 
	    }
...

{
    ...
    for(e=1;e<=E->mesh.nel;e++)  {
	  ...
	  for(i=1;i<=ppts;i++){
	    ...
	    visc[i] = 0.0;
	    for(j=1;j<=vpts;j++)
		  visc[i] +=  E->EVI[E->mesh.levmax][(e-1)*vpts+j];
        ...
        for(j=1;j<=ends;j++) {
          ...
          if (dims==3) VY[j] = E->V[3][E->ien[e].node[j]];
	      }

      for(j=1;j<=ends;j++)  {
          Szz += 2.0 * visc[1] * VZ[j]* GNx.ppt[GNPXINDEX(1,j,1)];
          Sxx += 2.0 * visc[1] * VX[j]* GNx.ppt[GNPXINDEX(0,j,1)];
	  Sxz += visc[1] * VX[j]* GNx.ppt[GNPXINDEX(1,j,1)] + visc[1] * VZ[j]* GNx.ppt[GNPXINDEX(0,j,1)];
          if (dims==3)   {
	      Syy += 2.0 * visc[1] * VY[j]* GNx.ppt[GNPXINDEX(2,j,1)];
	      Sxy += visc[1] * VX[j]* GNx.ppt[GNPXINDEX(2,j,1)] + visc[1] * VY[j]* GNx.ppt[GNPXINDEX(0,j,1)];
	      Szy += visc[1] * VY[j]* GNx.ppt[GNPXINDEX(1,j,1)] + visc[1] * VZ[j]* GNx.ppt[GNPXINDEX(2,j,1)];
	      }
	  }

      Szz -= P[e];  /* add the pressure term */
      Sxx -= P[e];  /* add the pressure term */
      if (dims==3) Syy -= P[e];  /* add the pressure term */
     
      for(j=1;j<=ends;j++) {
	    node = E->ien[e].node[j];
	    SZZ[node] += E->tw[node] * Szz;  
	    SXX[node] += E->tw[node] * Sxx;  
	    SXZ[node] += E->tw[node] * Sxz;  
            if (dims==3)   {
	      SYY[node] += E->tw[node] * Syy;  
	      SXY[node] += E->tw[node] * Sxy;  
	      SZY[node] += E->tw[node] * Szy;  
              }
        }
