  ...
  for (i=1;i<=elz;i++)  {
    ...
    for (k=1;k<=ely;k++)    
      for (j=1;j<=elx;j++)     {
            ...
            get_global_1d_shape_fn(E,E->mesh.levmax,el,&M,&dGamma,0);
...
void remove_average(E,X1,X2)
  ...
  length = X1[E->mesh.nox]-X1[1];
  total = 0.0;

  if (E->control.AXI)  {
    length =(X1[E->mesh.nox]-X1[1])*(X1[E->mesh.nox]-X1[1]);
    for (j=1;j<=E->mesh.elx;j++)   {
      total += 0.5*(X2[j+1]+X2[j])*(X1[j+1]+X1[j])*
             (X1[j+1]-X1[j]);
      }
    }
  else   {
