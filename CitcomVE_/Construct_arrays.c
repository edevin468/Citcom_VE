...
{	
  ...
  for (lev=E->mesh.levmax;lev>=E->mesh.levmin;lev--)  {
    ...
    for(element=1;element<=E->mesh.NEL[E->mesh.levmax];element++)
      if ( element%E->mesh.elz==0) {
        e ++;
        E->sien[e].node[1] = E->ien[element].node[4]/E->mesh.noz;
        E->sien[e].node[2] = E->ien[element].node[3]/E->mesh.noz;
        if (dims==3)   {
          E->sien[e].node[3] = E->ien[element].node[7]/E->mesh.noz;
          E->sien[e].node[4] = E->ien[element].node[8]/E->mesh.noz;
          }
...
{ 
    ...
    const int lms=loc_mat_size[E->mesh.nsd];
                   ...
                   /* build nonzero B_R for r d.o.f. for nodes on density boundaries */
    construct_B_R(E);
	...
	for(i=0;i<=(neq+1);i++) 
	    ...
	    assert( E->BI[level][j] != 0 /* diagonal of matrix = 0, not acceptable */);
            E->BI[level][j]  = (float) 1.0/(E->BI[level][j]+E->B_R[level][j]);   
	    }

      if (E->control.verbose)   {
        fprintf(stderr,"output stiffness matrix!!!\n");
        fprintf(E->fp,"level %d %d\n",level,nno);
        for(j=1;j<=nno;j++)     {
...
{ 
  ...
  int i,j,k,el,lev,a,nodea,crit2;
  ...
  for (j=1; j<=E->mesh.elz; j++)   { 
    el = j + (i-1)*E->mesh.elz;
    E->mat[el] = 1;
    if (j<E->viscosity.n410) {
         E->mat[el] = 2;
        }
    if (j<E->viscosity.nlm) {
         E->mat[el] = 3;
        }
    }
