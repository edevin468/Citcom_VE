...
{ 
    ...
    E->PO = (double *) malloc ((E->mesh.npno+1)*sizeof(double));
    ...
    E->buoyancy = (double *) malloc((E->mesh.nno+1)*sizeof(double));
  ...
  for(i=0;i<=5;i++)  {
    E->slice.load[i]      = (double *)malloc((E->mesh.nsf+2)*sizeof(double));
    E->slice.surf[i]      = (double *)malloc((E->mesh.nsf+2)*sizeof(double));
    for(j=1;j<=E->mesh.nsf;j++)  {
      E->slice.surf[i][j]=0.0;
      E->slice.load[i][j]=0.0;
      }
    }

    E->load.mag      = (double *)malloc((E->load.stage+3)*sizeof(double));
    E->load.time      = (float *)malloc((E->load.stage+3)*sizeof(float));
  ...
  for(j=1;j<=E->mesh.nsd;j++)
     E->X0[j] = (double *)  malloc((E->mesh.nno+1)*sizeof(double));

  E->Maxwelltime = (double *) malloc((E->mesh.nel+2)*sizeof(double));
  E->MaxwelltimeO = (double *) malloc((E->mesh.nel+2)*sizeof(double));
  E->ShearMO = (double *) malloc((E->mesh.nel+2)*sizeof(double));

  E->S2xx = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->S2zz = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->S2xz = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->S2yy = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->EVold = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->EVolder = (float *) malloc((E->mesh.nel+2)*sizeof(float));

  E->Exx = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->Exz = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->Ezz = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->Eyy = (float *) malloc((E->mesh.nel+2)*sizeof(float));

  E->Sxx = (double *) malloc((E->mesh.nel*4+2)*sizeof(double));
  E->Szz = (double *) malloc((E->mesh.nel*4+2)*sizeof(double));
  E->Sxz = (double *) malloc((E->mesh.nel*4+2)*sizeof(double));
  E->Syy = (double *) malloc((E->mesh.nel*4+2)*sizeof(double));
  E->Xsurf[1] = (double *)  malloc((E->mesh.nox+1)*sizeof(double));
  E->Xsurf[2] = (double *)  malloc((E->mesh.nox+1)*sizeof(double));

  E->EViO = (float *) malloc((E->mesh.nel+2)*vpoints[E->mesh.nsd]*sizeof(float));
  E->viscosity.element1 = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->viscosity.element = (float *) malloc((E->mesh.nel+2)*vpoints[E->mesh.nsd]*sizeof(float));
  E->viscosity.dissipation_e = (float *) malloc((E->mesh.nel+2)*sizeof(float));
  E->viscosity.dissipation_n = (float *) malloc((E->mesh.nno+2)*sizeof(float));

  for(i=E->mesh.levmin;i<=E->mesh.levmax;i++)   {
    for(j=1;j<=E->mesh.nsd;j++)   {
      E->XX[i][j] = (double *)  malloc((E->mesh.NNO[i]+1)*sizeof(double));
      E->Interp[i][j] = (double *)  malloc((E->mesh.NNO[i]+1)*sizeof(double));
      }
    E->MASS[i]     = (double *) malloc((E->mesh.NNO[i]+1)*sizeof(double));
...
*/
  ...
  for(i=1;i<=E->mesh.nel;i++)  {
      E->Maxwelltime[i] = 1.0; E->MaxwelltimeO[i] = 1.0; E->ShearMO[i]=1;
      E->EVolder[i] = 1.0; E->EVold[i] = 1.0;
      E->mat[i]=1;
      E->Exx[i]=0.0;  E->Ezz[i]=0.0; E->Exz[i]=E->Eyy[i]=0.0;
      E->S2zz[i]=0.0; E->S2xx[i]=0.0; E->S2yy[i]=0.0; E->S2xz[i]=0.0;
      }
  for(i=0;i<=E->mesh.nel*4;i++)  {
      E->Sxx[i]=0.0;  E->Szz[i]=0.0; E->Sxz[i]=0; E->Syy[i]=0.0;
      }
...
{ 
  ...
  for(i=E->mesh.levmin;i<=E->mesh.levmax;i++)  {
    ...
    E->B_R[i] = (higher_precision *) malloc((E->mesh.NEQ[i]+2)*sizeof(higher_precision)); 
     ...
     struct All_variables *E;
{
  ...
  E->viscosity.dissipation = 0;
  E->viscosity.dissipation_total = 0;
  E->viscosity.elastic_total = 0;
  E->viscosity.work_done = 0;
  E->viscosity.work_done_total = 0;
  ...
  E->control.VBZtopval=0.0;
  E->control.VBZbotval=0.0;

  E->sphere.radius = 6370000.0;
  ...
  
{
  ...
  if ( strcmp(E->control.NODE_SPACING,"regular") == 0)
    ...
    input_string("load_history_file",E->control.data_file1,"initialize");
    ...
    input_int("pure_visc",&(E->control.pure_visc),"0");
        /* for layers    */
    E->viscosity.nlm = 0;
    E->viscosity.n410 = 0;
    E->viscosity.zlm = 1.0;
    ...
    input_float("plateage",&(E->control.plate_age),"0.0");
    ...
    input_float("topvbzval",&(E->control.VBZtopval),"0.0");
    input_float("botvbzval",&(E->control.VBZbotval),"0.0");
    ...
    input_int("load_stages",&(E->load.stage),"nodefault");

    input_float("basin_w",&(E->mesh.bl1width[1]),"nodefault");
    input_float("weak_plug_s",&(E->mesh.bl1width[2]),"nodefault");
    input_float("weak_plug_e",&(E->mesh.bl1width[3]),"nodefault");
    input_float("Te_s",&(E->mesh.bl2width[2]),"nodefault");
    input_float("Te_e",&(E->mesh.bl2width[3]),"nodefault");
    ...
    input_double("box_height",&(E->sphere.ro),"nodefault");

// fixed at 2900 km
    E->sphere.ro=2.9e6;
    E->sphere.radius = E->sphere.ro;
  ...
  input_float("rotation_rate",&(E->data.rotation_rate),"1.0");
  ...
  input_float("core_density",&(E->data.density_below),"7150.0");
  ...
  E->data.rotation_rate = E->data.rotation_rate*2.0*M_PI/(24.0*3600.0);
