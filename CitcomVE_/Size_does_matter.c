...
void get_global_1d_shape_fn_o(E,el,GM,dGammax,nodal)
     struct All_variables *E;
     int el,nodal;
...
void get_global_1d_shape_fn(E,lev,el,GM,dGammax,nodal)
     struct All_variables *E;
     int el,nodal,lev;
     struct Shape_function1 *GM;
     struct Shape_function1_dA *dGammax;
	...
	for(e=1;e<=E->mesh.nsd-1;e++)
     ...
     if(nodal==0)  /* Gaussian */
        for(i=1;i<=onedvpoints[E->mesh.nsd];i++)      /* nodes */
           for(d=1;d<=E->mesh.nsd-1;d++)
	      for(e=1;e<=E->mesh.nsd-1;e++)    {
	          dxda[d][e] += E->XX[lev][1][E->IEN[lev][el].node[node[i]]]*E->Mx.vpt[GMVXINDEX(d-1,i,k)];      
	          }
     else if(nodal==1)  /* Lobatto */
        for(i=1;i<=onedvpoints[E->mesh.nsd];i++)      /* nodes */
           for(d=1;d<=E->mesh.nsd-1;d++)
              for(e=1;e<=E->mesh.nsd-1;e++)    {
	          dxda[d][e] += E->XX[lev][1][E->IEN[lev][el].node[node[i]]]*E->Lx.vpt[GMVXINDEX(d-1,i,k)];      
	          }

     jacobian = determinant(dxda,E->mesh.nsd-1); 
     dGammax->vpt[GMVGAMMA(dirn,k)] = jacobian;
     }
