...
{ 
    ...
    Ra = E->data.density*pow(E->sphere.radius,(double)4.0)*pow(E->data.rotation_rate,(double)2.0)/(E->data.ref_viscosity*E->data.therm_diff);

    Ra = 0.0;

    if (abs(E->control.Ra_670)>0.0)   {

      phase_change(E,E->Fas670,E->Fas670_b);

      for(i=1;i<=E->mesh.nno;i++)
        E->buoyancy[i] = -E->control.Ra_temp * E->T[i] + E->control.Ra_670 * E->Fas670[i];
      }
    else 
      for(i=1;i<=E->mesh.nno;i++)
        E->buoyancy[i] = Ra*E->T[i];  
