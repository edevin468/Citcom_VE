...
{ 
  ...
  double *shuffle, *R;
  double alpha,delta,s2dotAhat,r0dotr0,r1dotz1,dpressure,dvelocity;
  ...
  if (been_here==0 || E->mesh.nmx!=been_here) {
    stops = 0;
    }

  been_here = E->mesh.nmx;
  ...
  fprintf(E->fp,"initial residue of momentum equation F %.8e %d\n",v_res,gneq);
...
// while( count==0 || ((count < *steps_max) && (E->monitor.incompressibility >= E->control.tole_comp || dvelocity >= imp || dpressure >=imp) ) )  {   
 ...
 while( count==0 || ((count < *steps_max) && (dvelocity >= imp || dpressure >=imp) && E->monitor.incompressibility >=E->control.tole_comp ) )  {
     ...
     count++;
     if (count%5==0&&E->control.print_convergence )  {
